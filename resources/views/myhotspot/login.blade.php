...
@section('content')
                ...
                <div class="card-body ">
                    ...
                    <div class="tab-content">
                        <div id="member" class="tab-pane text-light mb-3 active">
                            ...
                            <form id="formlogout" action="http://{{ $myuamip }}:{{ $myuamport }}/json/logout" method="get"
                            class="mt-3">                            
                            ...
                            <div class="modal-body ">                           
                                ...
                                <div class="d-flex justify-content-center">
                                    <div class="alert alert-success w-100 text-center" role="alert">
                                        <h4><span id="ddhhmm">DD:HH:MM</span></h4>                                        
                                    </div>                                    
                                </div>
    ...
    @minify('js')
        ...
        function modalButtonCheckActive(){
            if($("#toggle-time").hasClass("active")){
                package_name = "time";
                console.log("time default");
                document.getElementById('ddhhmm').innerHTML = ConvertMinutes(time_value);
            } else if($("#toggle-data").hasClass("active")){
                package_name = "data";
                console.log("data default");
                document.getElementById('ddhhmm').innerHTML = data_value + "MB";
            } else {
                package_name = "charge";
                console.log("charge default");
                document.getElementById('ddhhmm').innerHTML = ConvertMinutes(charge_value);
            }
            if($('#toggle-autologin').hasClass("active")){
                service_name = "autologin";
                console.log("autologin default");
            }else{
        ...
        function modalButtonsEnabled(time,data,charge,service,default_package,default_service){
            if(time && !data && !charge){
                $('#toggle-package').hide();
            }
            if(!time){
            ...
            if(default_package == 0){
                $('#toggle-time').addClass('active');
            } else if(default_package == 1){
                $('#toggle-data').addClass('active');
            } else {
                $('#toggle-charge').addClass('active');
            }
            if(default_service == 0){
                $('#toggle-generate').addClass('active');
            }else{
                $('#toggle-autologin').addClass('active');
            }
        ...
        function autologinClick(id) {
            ...
            var default_packages = JSON.parse('@json($mydefaultpackages)');
            var default_services = JSON.parse('@json($mydefaultservices)');
            modalButtonsEnabled(time_enables[id],data_enables[id],charge_enables[id],generate_enables[id],default_packages[id],default_services[id])
            console.log(time_enables[id], data_enables[id], charge_enables[id], generate_enables[id], default_packages[id], default_services[id]);
            modalButtonCheckActive();
             if(states[id] == "down"){
                 notifier('Vendo '+ document.getElementById('btn'+id).innerHTML +' Offline',"error");
                 return;
             }
            ...
            if (!!window.EventSource) {
                if (time == 0) {
                    ...
                    source.addEventListener('message', function(e) {
                        ...
                        credit = data.credit;
                        queue = data.queue;
                        time_value = data.etime;
                        data_value = data.edata;
                        charge_value = data.echarge;
                    ...
                    setTimeout(() => {
                        if(queue){
                            notifier('You are on queue','warning');
                        } else {
                            notifier('Please Insert coin','success');
                        }                        
                    }, 1500);
