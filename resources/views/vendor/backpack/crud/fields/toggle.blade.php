        ...
        @php($optionPointer++)
            ...
            @if (isset($field['inline']) && $field['inline'])
                ...
                <label class="radio-inline" for="{{ $field['name'] }}_{{ $optionPointer }}">
                    <input data-field-toggle="{{ json_encode($field['hide_when']) }}" type="radio"
                        id="{{ $field['name'] }}_{{ $optionPointer }}" name="{{ $field['name'] }}"
                        value="{{ $value }}" {{ $optionValue !== '' && $optionValue == $value ? ' checked' : '' }}>
                    {!! $label !!}
                </label>
                ...
                <div class="radio">
                    <label for="{{ $field['name'] }}_{{ $optionPointer }}">
                        <input data-field-toggle="{{ json_encode($field['hide_when']) }}" type="radio"
                            id="{{ $field['name'] }}_{{ $optionPointer }}" name="{{ $field['name'] }}"
                            value="{{ $value }}"
                            {{ $optionValue !== '' && $optionValue == $value ? ' checked' : '' }}>
                        {!! $label !!}
                    </label>
                </div>
